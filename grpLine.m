...
%% defaults
% see the commented out parts in the code to see where these variables are
% used

fontsize = 22;
colmap = 'bone';
colmap = cubehelix(4,[2,-2,2.3,1],[0.2,0.8]);
line.tex = {'-','--','-.',':'};
...
% height = 16.2;                    % Height in inches
height = width*((sqrt(5)-1)/2);     % golden ratio height/width
% fsz = 11;                         % Fontsize for the gca (axis)
fsz = fontsize * ((sqrt(5)-1)/2);   % Use a golden ratio here
alw = 1.25;                         % AxesLineWidth
lw = 1.5;                           % LineWidth
msz = 8;                            % MarkerSize
noDataAxisPortion = 0.05; % re-adjust after axis tight above this % range
...
if nargin > 1 && exist('x', 'var') && isnumeric(x)
  ...
  if size(x, 2) > 2
    ...
    if exist('idx', 'var') && iscell(idx)
      % try converthing this to matrix
      warning('Converting group cell to matrix form');
      idx = str2double(idx);
    end
...
if nargin < 5 || ~exist('mode', 'var')
  if ~(exist('mode','var') && isfield(mode, 'errbar') && isnumeric(mode.errbar) )
    switch size(x,2)
      ...
      otherwise
        % TODO: 1xn vector produces the unicorn
...
if mode.isline
  ...
  if mode.errbar
    %         [hbar herrbar] = barwitherr(errdat, ydat, 'BarWidth',1);
    errorb(ydat, errdat);
    %      errorbar_groups(ydat, errdat);
  else
    bar(ydat);
  end
...
%% add in plot info, and last parameter adjustments
if numel(gnames)==1
...
% set the axis to reasonable size
axis tight;
xLim = get(gca, 'XLim');
yLim = get(gca, 'YLim');
dxLim = noDataAxisPortion * abs(diff(xLim));
dyLim = noDataAxisPortion * abs(diff(yLim));
axis([xLim(1)-dxLim, xLim(2)+dxLim, yLim(1)-dyLim, yLim(2)+dyLim]);
set(gca, 'Layer', 'top');
hold off;
